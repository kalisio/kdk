export default async function (app, options) {
  ...
  await options.Model.createIndex({ email: 1 }, { unique: true })
  ...
  await options.Model.createIndex({ 'profile.name': 1 }, { name: 'name-en', collation: { locale: 'en', strength: 1 } })
  await options.Model.createIndex({ 'profile.name': 1 }, { name: 'name-fr', collation: { locale: 'fr', strength: 1 } })
