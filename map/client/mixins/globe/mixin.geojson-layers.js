...
export const geojsonLayers = {
  ...
  methods: {
    ...
    async loadGeoJson (dataSource, geoJson, options, updateOptions = {}) {
      ...
      // Can create new features in some cases. These features will be tagged with a autoGenerated property
      // and need to be deleted on refresh (in memory layer) to avoid to duplicate them
      ...
      if (geoJson) {
        if (!_.has(geoJson, 'features')) {
          // If we have a single feature, wrap it in a FeatureCollection
          geoJson = {
