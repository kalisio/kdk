  ...
  methods: {
    ...
    async toGeoJson (name) {
      if (!this.isLayerVisible(name)) {
        ...
        if (cachedGeojson) {
          // Ensure that auto-generated features are removed
          _.set(cachedGeojson, 'features', _.filter(_.get(cachedGeojson, 'features', []), feature => !_.get(feature, 'properties.autoGenerated', false)))
      ...
      // Filter entities to remove auto-generated ones as they'll be re-generated
      const collection = new EntityCollection()
      const originalFeatures = _.filter(layer.entities.values, (entity) => {
        const property = _.get(entity, 'properties.autoGenerated')
        if (!property) return true
        return !property.getValue()
      })
      _.forEach(originalFeatures, entity => { collection.add(entity) })
      const geoJson = await convertEntitiesToGeoJson(collection)
      return geoJson
    },
